...
async function init() {
    ...
    try {
        ...
        setUi(globalConfig);
        loadEnd(globalConfig.categories || []);
        startWIBClock();
        loadReminder();
        setSearch();
        setupLinks(globalConfig.links || []);
        setupModalHandlers();
    } catch (e) {
...
function startWIBClock() {
    ...
    function updateTime() {
        const now = new Date();
        if (timeEl) timeEl.innerText = now.toLocaleTimeString('id-ID', { timeZone: 'Asia/Jakarta', hour12: false, hour: '2-digit', minute: '2-digit', second: '2-digit' });
        if (dateEl) dateEl.innerText = now.toLocaleDateString('id-ID', { timeZone: 'Asia/Jakarta', day: 'numeric', month: 'long', year: 'numeric' });
    }
}
...
function setUi(config) {
    const s = config;
    if (document.getElementById('header')) document.getElementById('header').innerText = s.name || 'HANZ API';
    if (document.getElementById('versionHeader')) document.getElementById('versionHeader').innerText = s.header?.status || 'ONLINE';
    if (document.getElementById('page')) document.getElementById('page').innerText = s.name || 'HANZ API';
    ...
    if (s.header?.imageSrc) {
        const imgSrcs = Array.isArray(s.header.imageSrc) ? s.header.imageSrc : [s.header.imageSrc];
        const randomSrc = imgSrcs.length > 0 ? imgSrcs[Math.floor(Math.random() * imgSrcs.length)] : '';
        const video = document.getElementById('dynamicImage');
        if (video) {
            const source = video.querySelector('source');
            if (source) {
                source.src = randomSrc;
                video.load();
            }
        }
    }
...
function setupLinks(links) {
    const container = document.querySelector('#apiLinks .space-y-1');
    if (!container) return;
    container.innerHTML = '';
    links.forEach(({ url, name }) => {
        ...
        a.href = url;
        a.target = '_blank';
        a.className = 'block text-xs text-slate-600 hover:text-fmn-blue transition-colors py-1 truncate';
        a.innerHTML = `<i class="fa-solid fa-arrow-up-right-from-square mr-2 text-[10px]"></i>${name}`;
        ...
        section.innerHTML = `
                <button id="btn-${catId}" onclick="toggleCategory('${catId}')" class="category-btn w-full flex items-center justify-between bg-white text-slate-700 p-4 rounded-xl shadow-card border border-slate-200 hover:border-slate-300 transition-all duration-300 group mb-3">
                    <div class="relative z-10 flex items-center gap-4 w-full">
                        ...
                        <h2 class="text-base font-semibold tracking-wide text-slate-800 flex-1 text-left">${catName}</h2>
                        <div class="flex items-center gap-3">
                            <span class="text-[10px] font-mono bg-slate-100 text-slate-500 border border-slate-200 px-2 py-1 rounded font-semibold transition-colors group-hover:bg-fmn-blue/10 group-hover:text-fmn-dark group-hover:border-fmn-blue/20">${sortedItems.length} EPS</span>
                ...
                <div id="grid-${catId}" class="api-section-grid grid grid-cols-1 md:grid-cols-2 gap-2 hidden mb-6 pl-1 w-full"></div>
            `;

        const grid = section.querySelector('.api-section-grid');

        sortedItems.forEach((item, idx) => {
            const id = `${catName}-${idx}`.replace(/\s+/g, '-');
            const endpoint = item.path || '';
            const method = 'GET';
            const name = item.name || 'Unknown';
            const desc = item.desc || '';
            const innerDesc = item.innerDesc || '';

            const methodColor = 'bg-sky-500 text-white border-sky-600';
            ...
            if (endpoint.includes('?')) {
                ...
                urlParams.forEach((_, key) => {
                    params.push({ name: key, required: true });
                });
                copyUrl = endpoint;
            }
                ...
                inputs = `<div class="bg-slate-50 p-4 border-y border-slate-100 grid gap-4 w-full animate-fade-in-up">` +
                    params.map(p => `
                            <div class="min-w-0 group/input">
                                <label class="text-[11px] font-semibold text-slate-500 tracking-wide mb-1.5 block transition-colors group-focus-within/input:text-fmn-blue">${p.name} <span class="text-red-400">*</span></label>
                                <input type="text" id="input-${id}-${p.name}" placeholder="Enter ${p.name}..." class="w-full min-w-0 border border-slate-200 p-2.5 text-sm rounded-lg focus:border-fmn-blue focus:ring-4 focus:ring-fmn-blue/10 focus:outline-none bg-white shadow-sm max-w-full transition-all">
                            </div>
                        `).join('') +
                    (innerDesc ? `<div class="text-xs text-slate-500 mt-1 whitespace-pre-wrap">${innerDesc}</div>` : '') +
                    `</div>`;
            }
            ...
            card.setAttribute('data-search', `${name} ${endpoint} ${catName}`);
            ...
            card.innerHTML = `
                        ...
                        <div class="flex justify-between items-center gap-3">
                            <div class="flex items-center gap-3 overflow-hidden min-w-0 flex-1">
                                ...
                                <div class="flex flex-col min-w-0 flex-1">
                                    ...
                                    <span class="text-[11px] text-slate-500 mt-0.5 truncate">${desc}</span>
                                </div>
                            </div>
                    ...
                    <div id="body-${id}" class="hidden animate-slide-down w-full">
                        ...
                        <div id="res-area-${id}" class="response-area hidden mx-4 mb-4 rounded-xl overflow-hidden bg-term-bg border border-slate-700 shadow-inner relative max-w-[calc(100%-2rem)]">
                            <div class="bg-term-header px-4 py-2 flex items-center justify-between border-b border-slate-700/50 flex-wrap gap-2">
...
function setupModalHandlers() {
    ...
    modalEl.addEventListener('hidden.bs.modal', () => {
        ...
        document.getElementById('submitQueryBtn')?.classList.add('d-none');
    });
}
