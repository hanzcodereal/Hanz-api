...
module.exports = (app) => {
    app.get('/youtube-music/lyrics', async (req, res) => {
        ...
        try {
            ...
            try { 
                ...
                title=getRunsText(c.title?.runs||[]); 
                artist=getRunsText(c.shortBylineText?.runs||c.longBylineText?.runs||[]); 
                album=getRunsText(c.longBylineText?.runs||[]); 
            } catch(e){}
            ...
            if(title && artist){
                try {
                    const lrc = await makeRequest({
                        hostname: 'lrclib.net',
                        path: '/api/search?q=' + encodeURIComponent(title + ' ' + artist),
                        method: 'GET',
