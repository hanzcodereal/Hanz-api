...
module.exports = (app) => {
    app.get('/download/tiktok/v2', async (req, res) => {
        ...
        if (typeof url !== "string" || url.trim().length === 0) {
            return res.status(400).json({
                status: false,
                error: "URL harus berupa string yang tidak kosong",
                code: 400
            });
        }
        ...
        try {
            ...
            const result = await scraper.scrape(url.trim());

            if (result && result.error) {
                return res.status(500).json({
                    status: false,
                    error: result.message || "Gagal mengambil data TikTok",
                    code: 500
                });
            }
    ...
    app.post('/download/tiktok/v2', async (req, res) => {
